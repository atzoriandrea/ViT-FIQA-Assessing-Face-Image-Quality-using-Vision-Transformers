...
def get_dataloader(
    ...
    # LMDB file
    elif root_dir.endswith(".lmdb_dataset"):
        train_set = LmdbDataset(lmdb_file=root_dir, augmentation=augmentation, augmentation_params = augmentation_params)
    # Mxnet RecordIO
    elif os.path.exists(rec) and os.path.exists(idx):
        train_set = MXFaceDataset(root_dir=root_dir, local_rank=local_rank, augmentation=augmentation, augmentation_params = augmentation_params)
...
class MXFaceDataset(Dataset):
    def __init__(self, root_dir, local_rank, augmentation=None, augmentation_params=None):
        ...
        if augmentation == "basic":
            ...
            augmenter = GridSampleAugmenter(augmentation_params, input_size=112, p=0.4)
            self.transform = transforms.Compose(
                [transforms.ToPILImage(),
                 augmenter,
                 transforms.ToTensor(),
            ...
            self.transform = transforms.Compose(aug_rand_4_16)
        else:
            self.transform = transforms.Compose(
                [transforms.ToPILImage(),
                 transforms.RandomHorizontalFlip(),

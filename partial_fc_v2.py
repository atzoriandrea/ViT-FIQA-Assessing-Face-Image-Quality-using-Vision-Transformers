...
class PartialFC_V2(torch.nn.Module):
    ...
    def __init__(
            ...
            fp16: bool = False, alpha: float = 10.0
    ):
        """
        Paramenters:
        -----------
        ...
        self.criterion_qs = torch.nn.SmoothL1Loss(beta=0.5, reduction="none")
        self.dist_probability = DistCrossEntropy()
    ...
    def forward(
        ...
        _gather_qs = [
            torch.zeros((batch_size, 1)).cuda() for _ in range(self.world_size)
        ]
        _list_embeddings = AllGather(local_embeddings, *_gather_embeddings)
        distributed.all_gather(_gather_labels, local_labels)
        _list_qs = AllGather(loacl_qs, *_gather_qs)

        embeddings = torch.cat(_list_embeddings)
        labels = torch.cat(_gather_labels)
        qs = torch.cat(_list_qs)
        ...
        return loss + self.alpha * partial_fiqa, partial_fiqa


def all_gather_qs(q, ws, device):
    """
    ...
    distributed.all_gather(all_qs_padded, q)
    all_qs = []
...
def Distprobaility(logits: torch.Tensor, label: torch.Tensor, qs: torch.Tensor, smotthl1):
    """ """
    ...
    # qs_index = torch.zeros(batch_size, 1, device=qs.device)

    loss[index] = logits[index].gather(1, label[index])
    # qs_index[index] = qs[index].gather(1,label[index])
    # print(loss.size())
    # print(qs.size())
    # loss_qs=  smotthl1(loss,qs)

    distributed.all_reduce(loss, distributed.ReduceOp.SUM)
    loss = smotthl1(loss, qs)
